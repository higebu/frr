...
def test_dsd_policy_owns_only_its_dt_sid():
    ...
    `default` locator r1 must therefore hold exactly two bgpd SIDs:
    slice1's End.M.GTP4.E (ISD, IPv4 AF) and slice2's End.DT4 (DSD).
    A third one means the `segment direct` policy also pulled an
    End.M.GTP4.E SID it never advertises."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def _two_sids(router):
        sids = _bgp_sids_under(router, "2001:db8:e::/48")
        if len(sids) == 2:
            return None
        return (
            "expected exactly two bgpd SIDs under 2001:db8:e::/48 "
            "(slice1 End.M.GTP4.E + slice2 End.DT4), got {}".format(sids)
        )
    ...
    test_func = functools.partial(_two_sids, r1)
