...
def test_running_config_reloads_cleanly():
    """Feeding r1's own `show running-config` back through `vtysh -f`
    must not have any daemon reject a line.  Guards the `segment
    direct` sub-block, whose `exit` must land bgpd back in the MUP
    address-family node so the following `exit-address-family` is
    accepted."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    r1 = tgen.gears["r1"]
    running = r1.vtysh_cmd("show running-config no-header")
    result = r1.vtysh_multicmd(running, pretty_output=False)
    rejected = [
        line
        for line in result.splitlines()
        if "Unknown command" in line or "processing failure" in line
    ]
    assert not rejected, (
        "running-config round-trip rejected lines:\n{}\n--- config ---\n{}".format(
            "\n".join(rejected), running
        )
    )
...
def _bgp_sids_under(router, locator_prefix):
    """Return the default-VRF `show ipv6 route` prefixes that fall
    inside `locator_prefix` and were installed by bgpd, i.e. the local
    SIDs this router's MUP origination owns."""
    import ipaddress

    net = ipaddress.ip_network(locator_prefix)
    out = router.vtysh_cmd("show ipv6 route json", isjson=True)
    sids = []
    for prefix, routes in (out or {}).items():
        if ipaddress.ip_network(prefix).subnet_of(net) and any(
            r.get("protocol") == "bgp" for r in routes
        ):
            sids.append(prefix)
    return sorted(sids)


def test_dsd_policy_owns_only_its_dt_sid():
    """`sid auto` is shared by both origination modes, but only
    `segment interwork` may claim an End.M.GTP*.E SID.  Under the
    `default` locator r1 must therefore hold exactly three bgpd SIDs:
    slice1's End.M.GTP4.E (ISD, IPv4 AF), slice2's End.DT4 for the
    VPNv4 export and slice2's End.DT4 for the DSD.  A fourth one means
    the `segment direct` policy also pulled an End.M.GTP4.E SID it
    never advertises."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def _three_sids(router):
        sids = _bgp_sids_under(router, "2001:db8:e::/48")
        if len(sids) == 3:
            return None
        return (
            "expected exactly three bgpd SIDs under 2001:db8:e::/48 "
            "(slice1 End.M.GTP4.E + slice2 VPNv4 End.DT4 + slice2 DSD "
            "End.DT4), got {}".format(sids)
        )

    # A stray allocation shows up right away; a SID released by a policy
    # that stopped owning it can take a moment to leave the RIB.
    test_func = functools.partial(_three_sids, r1)
    _, result = topotest.run_and_expect(test_func, None, count=30, wait=1)
    assert result is None, result


def test_isd_localsid_carries_source_upf_prefix_len():
    """The End.M.GTP4.E local SID r1 installs for its ISD policy must be
    programmed with the `source-upf-prefix-len 56` of that address
    family, so the gateway reads the IPv4 source back from the offset
    the far PE wrote it at."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def _gtp4e_prefix_len(router):
        seen = []
        for sid in _bgp_sids_under(router, "2001:db8:e::/48"):
            out = router.vtysh_cmd("show ipv6 route {} json".format(sid), isjson=True)
            for paths in (out or {}).values():
                for path in paths if isinstance(paths, list) else []:
                    for nh in path.get("nexthops", []):
                        if nh.get("seg6mobile", {}).get("action") != "End.M.GTP4.E":
                            continue
                        got = nh.get("seg6mobileContext", {}).get("v6SrcPrefixLen")
                        seen.append((sid, got))
                        if got == 56:
                            return None
        return "no End.M.GTP4.E local SID with v6SrcPrefixLen 56; saw {}".format(seen)

    def _gtp4e_prefix_len_is(router, want):
        got = None
        for sid in _bgp_sids_under(router, "2001:db8:e::/48"):
            out = router.vtysh_cmd("show ipv6 route {} json".format(sid), isjson=True)
            for paths in (out or {}).values():
                for path in paths if isinstance(paths, list) else []:
                    for nh in path.get("nexthops", []):
                        ...
                        if got == want:
                            return None
        return "End.M.GTP4.E local SID has v6SrcPrefixLen {}, want {}".format(got, want)
    ...
    _wait_for(r1, _gtp4e_prefix_len, "End.M.GTP4.E local SID carries v6SrcPrefixLen 56")

    # Changing the length must re-program the endpoint on its own: the
    # policy is otherwise untouched, so an install keyed only on the SID
    # and the action would skip the update and leave the gateway reading
    # the IPv4 source at the old offset.
    _set_len("source-upf-prefix-len 48")
    _wait_for(r1, lambda r: _gtp4e_prefix_len_is(r, 48),
              "End.M.GTP4.E local SID follows the new length")

    _set_len("no source-upf-prefix-len")
    _wait_for(r1, lambda r: _gtp4e_prefix_len_is(r, 56),
              "End.M.GTP4.E local SID falls back to the default length")

    _set_len("source-upf-prefix-len 56")
    _wait_for(r1, lambda r: _gtp4e_prefix_len_is(r, 56),
              "End.M.GTP4.E local SID restored")


def _zebra_dt4_sids_in_vrf(router, vrf_name):
    """Return the SIDs zebra's SID manager holds as End.DT4 for
    `vrf_name`, from `show segment-routing srv6 sid json`."""
    out = router.vtysh_cmd("show segment-routing srv6 sid json", isjson=True)
    sids = []
    for sid, entry in (out or {}).items():
        if entry.get("behavior") != "End.DT4":
            continue
        if entry.get("context", {}).get("vrfName") != vrf_name:
            continue
        sids.append(sid)
    return sorted(sids)
...
def _dsd_sid(router):
    """The SID r1 advertises on its slice2 DSD, or None."""
    out = router.vtysh_cmd("show bgp ipv4 mup all detail-routes")
    in_dsd = False
    for line in out.splitlines():
        if "[2]:[12]:[10.0.0.250]" in line:
            in_dsd = True
        elif in_dsd and "Remote SID:" in line:
            return line.split("Remote SID:")[1].split()[0].rstrip(",")
    return None


def test_dsd_sid_distinct_from_l3vpn_sid():
    """slice2 asks the SID manager for an End.DT4 twice - once for the
    VPNv4 `sid vpn export auto`, once for the DSD `sid auto`.  Both
    requests carry the same behavior and vrf, so zebra must key them
    apart by the requesting address family: r1 has to end up with two
    different End.DT4 SIDs in slice2, and withdrawing the VPNv4 one
    must not take the DSD's SID with it."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def _two_distinct(router):
        sids = _zebra_dt4_sids_in_vrf(router, "slice2")
        if len(sids) == 2:
            return None
        return "expected two End.DT4 SIDs in slice2, got {}".format(sids)

    _wait_for(r1, _two_distinct, "two distinct slice2 End.DT4 SIDs")

    dsd_sid = _dsd_sid(r1)
    assert dsd_sid is not None, "slice2 DSD carries no SID"
    zebra_sids = _zebra_dt4_sids_in_vrf(r1, "slice2")
    assert dsd_sid in zebra_sids, "DSD SID {} not among zebra's slice2 End.DT4 {}".format(
        dsd_sid, zebra_sids
    )

    vpn_out = r1.vtysh_cmd("show bgp segment-routing srv6")
    vpn_sid = None
    in_slice2 = False
    for line in vpn_out.splitlines():
        if line.startswith("- name: "):
            in_slice2 = line.strip() == "- name: slice2"
        elif in_slice2 and "vpn_policy[AFI_IP].tovpn_sid:" in line:
            vpn_sid = line.split(":", 1)[1].strip()
    assert vpn_sid and vpn_sid != "(null)", "slice2 VPNv4 export SID not armed"
    assert vpn_sid != dsd_sid, "VPNv4 export and DSD share SID {}".format(dsd_sid)
    ...
    def _dsd_survives(router):
        sids = _zebra_dt4_sids_in_vrf(router, "slice2")
        if sids == [dsd_sid]:
            return None
        return "expected only the DSD SID {} left in slice2, got {}".format(
            dsd_sid, sids
        )

    _wait_for(r1, _dsd_survives, "DSD SID survives VPNv4 SID release")
    assert _dsd_sid(r1) == dsd_sid, "DSD lost or changed its SID"
...
def _t1st_encap_source(router, prefix, vrf):
    """The outer IPv6 source zebra holds on the SRv6 nexthop of the
    installed T1ST route, or None."""
    ...
    for paths in (out or {}).values():
        for path in paths if isinstance(paths, list) else []:
            if path.get("protocol") != "bgp":
                continue
            for nh in path.get("nexthops", []):
                if nh.get("srv6EncapSource"):
                    return nh["srv6EncapSource"]
    return None


def _figure10_source(prefix, ipv4_source):
    """RFC 9433 Section 6.6 Figure 10: Source UPF Prefix, then the IPv4
    Source Address right after the prefix length, then zero bits."""
    import ipaddress

    net = ipaddress.ip_network(prefix)
    shift = 128 - net.prefixlen - 32
    value = int(net.network_address) | (int(ipaddress.ip_address(ipv4_source)) << shift)
    return ipaddress.ip_address(value).compressed


def test_t1st_outer_source_follows_source_upf_prefix_len():
    """The T1ST H.Encaps must carry a per-route outer IPv6 source built
    from the T1ST Source Address (10.0.2.105, draft Section 3.3.9) laid
    out per Figure 10: r2's own locator 2001:db8:22::/48 as the Source
    UPF Prefix, with the IPv4 address at the configured
    `source-upf-prefix-len 56`, and at the default length once the
    command is removed."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def _expect(router, want):
        got = _t1st_encap_source(router, "192.168.1.5/32", "slice1")
        return None if got == want else "outer source {} != {}".format(got, want)

    want = _figure10_source("2001:db8:22::/56", "10.0.2.105")
    _wait_for(r2, lambda r: _expect(r, want),
              "T1ST outer source at source-upf-prefix-len 56")

    r2.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65002 vrf slice1\n"
        " address-family ipv4 mup\n"
        "  no source-upf-prefix-len\n"
        " exit-address-family\n"
        "exit\n"
    )
    want = _figure10_source("2001:db8:22::/56", "10.0.2.105")
    _wait_for(r2, lambda r: _expect(r, want),
              "T1ST outer source at the default length")
...
def _encap_behavior(router, prefix, vrf):
    """The SRv6 headend behavior zebra reports on the nexthop of the
    installed route: the string, None when the route is there but reports
    none, or "no-route" when there is no BGP nexthop to report on."""
    out = router.vtysh_cmd(
        "show ip route vrf {} {} json".format(vrf, prefix), isjson=True
    )
    for paths in (out or {}).values():
        for path in paths if isinstance(paths, list) else []:
            if path.get("protocol") != "bgp":
                continue
            for nh in path.get("nexthops", []):
                return nh.get("srv6EncapBehavior")
    return "no-route"


def test_t1st_encap_behavior_follows_srv6_knob():
    """The T1ST encapsulation must follow `encap-behavior` on the default
    instance's `segment-routing srv6` node (RFC 9433 Section 5.3.2.2 uses
    H.Encaps.Red for the downlink), while the T2ST H.M.GTP4.D route,
    which builds its own SRH, reports no headend behavior at all."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def _expect(prefix, want):
        def _check(router):
            got = _encap_behavior(router, prefix, "slice1")
            if got == want:
                return None
            return "{} encap behavior {} != {}".format(prefix, got, want)

        return _check

    _wait_for(r2, _expect("192.168.1.5/32", "H.Encaps"),
              "T1ST at the H.Encaps default")
    _wait_for(r2, _expect("10.0.0.250/32", None),
              "T2ST without a headend behavior")
    ...
    _wait_for(r2, _expect("192.168.1.5/32", "H.Encaps.Red"),
              "T1ST switched to H.Encaps.Red")
    _wait_for(r2, _expect("10.0.0.250/32", None),
              "T2ST unaffected by the knob")

    # zebra's own bookkeeping is not the kernel; check the netlink encoding.
    fib = r2.cmd("ip route show vrf slice1 192.168.1.5")
    assert "mode encap.red" in fib, "kernel route not reduced: {}".format(fib)

    # `no encap-behavior H_Encaps` is a deliberate no-op; the reset has to
    # name the value that is actually set.
    r2.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65002\n"
        " segment-routing srv6\n"
    ...
    _wait_for(r2, _expect("192.168.1.5/32", "H.Encaps"),
              "T1ST back at the H.Encaps default")
