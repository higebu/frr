...
def _encap_behavior(router, prefix, vrf):
    """The SRv6 headend behavior zebra holds on the nexthop of the
    installed route.  zebra omits the key at the H.Encaps default, so an
    absent key reads back as "H.Encaps"."""
    out = router.vtysh_cmd(
        "show ip route vrf {} {} json".format(vrf, prefix), isjson=True
    )
    for paths in (out or {}).values():
        for path in paths if isinstance(paths, list) else []:
            if path.get("protocol") != "bgp":
                continue
            for nh in path.get("nexthops", []):
                return nh.get("srv6EncapBehavior", "H.Encaps")
    return None
...
def test_t1st_encap_behavior_follows_srv6_knob():
    ...
    which builds its own SRH, stays on H.Encaps."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    _wait_for(r2, _expect("10.0.0.250/32", "H.Encaps"),
              "T2ST at the H.Encaps default")
