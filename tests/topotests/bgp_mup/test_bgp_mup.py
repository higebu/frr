...
def test_export_mup_pause_resume():
    """`no export mup` must atomically withdraw all leaked ISD/DSD
    NLRIs from the MUP RIB while leaving rd/rt/sid/segment policy
    intact in running-config.  `export mup` re-enables and the same
    NLRIs return.  Mirrors L3VPN's `import vpn` / `export vpn` master
    toggles (bgp_vty.c:bgp_imexport_vpn_cmd)."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    # Sanity: prior tests have left the steady-state ISD on r2.
    assert _grep(r2, "show bgp ipv4 mup all", "10.99.0.0/24"), (
        "precondition: ISD 10.99.0.0/24 must be present on r2"
    )

    # Pause the export.  rd/rt/sid lines stay in running-config; the
    # leaked NLRIs withdraw from r1's local MUP RIB and propagate the
    # withdraw to r2.
    r1.vtysh_cmd(
        "configure terminal\n"
    ...
    _wait_for(
        r1,
        lambda r: None
        if not _grep(r, "show bgp ipv4 mup all", "10.99.0.0/24")
        else "ISD still present on r1 after `no export mup`",
        "no export mup: ISD withdrawn from r1",
    )
    _wait_for(
        r2,
        lambda r: None
        if not _grep(r, "show bgp ipv4 mup all", "10.99.0.0/24")
        else "ISD still present on r2 after `no export mup`",
        "no export mup: ISD withdrawn from r2",
    )

    # Policy survives: rd/rt/sid lines still in r1's running-config.
    output = r1.vtysh_cmd("show running-config")
    for line in (
        "rd mup export 100:100",
        "rt mup export 65001:1",
        ...
        assert line in output, (
            "running-config dropped policy line `{}` after `no export mup`"
            .format(line)
        )
    # The explicit `no export mup` line itself round-trips.
    assert "no export mup" in output, (
        "running-config missing `no export mup` line after explicit toggle off"
    )
    ...
    _wait_for(
        r2,
        lambda r: None
        if _grep(r, "show bgp ipv4 mup all", "10.99.0.0/24")
        else "ISD did not return on r2 after `export mup`",
        "export mup: ISD re-leaked to r2",
    )

    # Now that the operator typed `export mup` explicitly, writeback
    # emits the explicit line (no longer suppressed by auto-set).
    output = r1.vtysh_cmd("show running-config")
    assert "\n  export mup\n" in output, (
        "running-config missing explicit `export mup` line after re-enable"
    )


def test_export_mup_staged_enable():
    """After the operator has explicitly typed `[no] export mup`,
    subsequent rd/rt/sid edits must NOT re-trigger the auto-set —
    auto-set only fires on configs where the operator has never
    typed the toggle explicitly.  This is the staged-enable scenario:
    clear the toggle, mutate policy, only then enable atomically."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    # Pause first so EXPLICIT latches.  Withdraw any current leak.
    r1.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65001 vrf slice1\n"
        " address-family ipv4 unicast\n"
    ...
    _wait_for(
        ...
        lambda r: None
        if not _grep(r, "show bgp ipv4 mup all", "10.99.0.0/24")
        else "precondition: ISD must withdraw before staged-enable test",
        "staged-enable precondition: leak paused",
    )

    # Mutate the RT — under auto-set this would have re-enabled the
    # leak immediately; with EXPLICIT latched the toggle stays clear.
    r1.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65001 vrf slice1\n"
        " address-family ipv4 unicast\n"
        "  rt mup export 65001:11\n"
        " exit-address-family\n"
        "exit\n"
    )

    # Give the route-map / leak a chance to fire (they shouldn't),
    # then assert nothing leaked.
    time.sleep(2)
    assert not _grep(r2, "show bgp ipv4 mup all", "10.99.0.0/24"), (
        "ISD leaked while `no export mup` was latched — staged-enable broken"
    )

    # Now flip the toggle on.  Leak should fire atomically with the
    # already-staged RT.
    r1.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65001 vrf slice1\n"
    ...
    _wait_for(
        ...
        lambda r: None
        if _grep(r, "show bgp ipv4 mup all", "10.99.0.0/24")
        else "leak did not start after `export mup` re-enable",
        "staged-enable: leak begins on `export mup`",
    )

    # Restore baseline RT for downstream tests.
    r1.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65001 vrf slice1\n"
        " address-family ipv4 unicast\n"
...
def test_import_mup_pause_resume():
    """`no import mup` must stop the per-vrf install of received MUP
    NLRIs (T1ST/T2ST) while leaving `rt mup import` policy intact.
    `import mup` resumes installation."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    # Sanity: T2ST 10.0.0.250 should currently be installed in slice1's
    # FIB (steady-state DSD/T2ST install from earlier tests).
    _wait_for(
        r2,
        lambda r: None
        if _route_via_bgp(r, "10.0.0.250/32", vrf="slice1")
        else "precondition: T2ST FIB entry must be present in slice1",
        "import-mup precondition",
    )
    ...
    _wait_for(
        ...
        else "T2ST FIB entry survived `no import mup`",
        "no import mup: install removed",
    )

    # `rt mup import` line still present.
    output = r2.vtysh_cmd("show running-config")
    assert "rt mup import 65001:1" in output, (
        "running-config dropped `rt mup import` after `no import mup`"
    )
    assert "no import mup" in output, (
        "running-config missing `no import mup` line"
    )
    ...
    _wait_for(
        ...
        lambda r: None
        if _route_via_bgp(r, "10.0.0.250/32", vrf="slice1")
        else "T2ST FIB entry did not return after `import mup`",
        "import mup: install resumed",
    )
