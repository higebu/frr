...
DEFUNSH(VTYSH_BGPD, address_family_ipv4_mup, address_family_ipv4_mup_cmd,
	"address-family ipv4 mup",
	"Enter Address Family command mode\n"
	BGP_AF_STR
	BGP_AF_MODIFIER_STR)
{
	vty->node = BGP_IPV4_MUP_NODE;
	return CMD_SUCCESS;
}

DEFUNSH(VTYSH_BGPD, address_family_ipv6_mup, address_family_ipv6_mup_cmd,
	"address-family ipv6 mup",
	"Enter Address Family command mode\n"
	BGP_AF_STR
	BGP_AF_MODIFIER_STR)
{
	vty->node = BGP_IPV6_MUP_NODE;
	return CMD_SUCCESS;
}

DEFUNSH(VTYSH_BGPD, bgp_segment_direct_mup, bgp_segment_direct_mup_cmd,
	"segment direct",
	"Origination mode for this MUP policy\n"
	"Direct Segment Discovery sub-block\n")
{
	if (vty->node == BGP_IPV4_MUP_NODE)
		vty->node = BGP_IPV4_MUP_SEGMENT_DIRECT_NODE;
	else if (vty->node == BGP_IPV6_MUP_NODE)
		vty->node = BGP_IPV6_MUP_SEGMENT_DIRECT_NODE;
	return CMD_SUCCESS;
}
...
void vtysh_init_vty(void)
{
	...
	install_element(BGP_NODE, &address_family_ipv4_mup_cmd);
	install_element(BGP_IPV4_MUP_NODE, &vtysh_quit_bgpd_cmd);
	install_element(BGP_IPV4_MUP_NODE, &vtysh_exit_bgpd_cmd);
	install_element(BGP_IPV4_MUP_NODE, &vtysh_end_all_cmd);
	install_element(BGP_IPV4_MUP_NODE, &exit_address_family_cmd);

	install_element(BGP_NODE, &address_family_ipv6_mup_cmd);
	install_element(BGP_IPV6_MUP_NODE, &vtysh_quit_bgpd_cmd);
	install_element(BGP_IPV6_MUP_NODE, &vtysh_exit_bgpd_cmd);
	install_element(BGP_IPV6_MUP_NODE, &vtysh_end_all_cmd);
	install_element(BGP_IPV6_MUP_NODE, &exit_address_family_cmd);

	install_element(BGP_IPV4_MUP_NODE, &bgp_segment_direct_mup_cmd);
	install_element(BGP_IPV6_MUP_NODE, &bgp_segment_direct_mup_cmd);
	install_element(BGP_IPV4_MUP_SEGMENT_DIRECT_NODE, &vtysh_quit_bgpd_cmd);
	install_element(BGP_IPV4_MUP_SEGMENT_DIRECT_NODE, &vtysh_exit_bgpd_cmd);
	install_element(BGP_IPV4_MUP_SEGMENT_DIRECT_NODE, &vtysh_end_all_cmd);
	install_element(BGP_IPV6_MUP_SEGMENT_DIRECT_NODE, &vtysh_quit_bgpd_cmd);
	install_element(BGP_IPV6_MUP_SEGMENT_DIRECT_NODE, &vtysh_exit_bgpd_cmd);
	install_element(BGP_IPV6_MUP_SEGMENT_DIRECT_NODE, &vtysh_end_all_cmd);
#endif /* HAVE_BGPD */
