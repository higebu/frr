	...
	for (; mpinfo; mpinfo = bgp_path_info_mpath_next(mpinfo)) {
		...
		*mpinfo_cp = *mpinfo;
		...
		if (bgp_attr_get_mup_fwd(mpinfo_cp->attr)) {
			...
			/* zapi_route_init() leaves the nexthop array alone, so
			 * the slot still holds the previous announce's fields.
			 */
...
static int bgp_zebra_srv6_sid_notify(ZAPI_CALLBACK_ARGS)
{
	...
	case ZAPI_SRV6_SID_ALLOCATED:
		...
		/* BGP-MUP gets first shot at SIDs requested on behalf of
		 * the MUP AF; everything else continues with the legacy
		 * seg6_local DT4/DT6/DT46 handling below.
		 */
