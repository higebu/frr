...
#define BGP_MUP_EXPORT_SID_REQUESTED (1u << 3)
			      ...
			      const struct in6_addr *sid_value)
{
	 ...
	 * shared End.M.GTP{4,6}.E SID.  If so, latch the SID on the
	 * policy and re-emit ISD NLRIs from the current unicast RIB.
	 */
...

	{
		...
		ep->flags &= ~BGP_MUP_EXPORT_SID_REQUESTED;
		if (to_bgp)
...
static bool mup_leak_should_skip_prefix(const struct prefix *p)
{
	if (!p)
		return true;
	/* Default route is not an ISD candidate per draft §3.3.1; ISD
...
static bool mup_export_request_sid_if_needed(struct bgp *from_bgp, afi_t afi,
					     struct bgp_mup_export_policy *ep)
{
	struct srv6_sid_ctx ctx = {};
	...
	if (!bgp_srv6_locator_lookup(from_bgp, bgp_get_default()))
		return false;
	if (ep->flags & BGP_MUP_EXPORT_SID_REQUESTED)
		return false;
	ctx.behavior = (afi == AFI_IP) ? ZEBRA_SEG6_LOCAL_ACTION_END_M_GTP4_E
				       : ZEBRA_SEG6_LOCAL_ACTION_END_M_GTP6_E;
	ctx.vrf_id = from_bgp->vrf_id;
	ep->flags |= BGP_MUP_EXPORT_SID_REQUESTED;
	bgp_zebra_request_srv6_sid(&ctx, &dummy, from_bgp->srv6_locator_name, NULL);
	return false;
}
			      ...
			      struct bgp_path_info *path_vrf)
{
	...
	if (!mup_export_request_sid_if_needed(from_bgp, afi, ep))
       ...
       "Route Distinguisher (<as-number>:<number> | <ip-address>:<number>)\n")
{
	...
	if (yes) {
		...
		ep->flags &= ~BGP_MUP_EXPORT_RD_SET;
		/* In-flight zebra SID request can never complete into this
		 * policy now that the RD half is gone; clear REQUESTED so a
		 * later re-config doesn't get stuck behind a stale latch.
		 * Keep the (potentially still-valid) tovpn_sid — the operator
		 * may re-add RD with a new value and reuse the SID.
		 */
		if (!ep->tovpn_sid_ready)
			ep->flags &= ~BGP_MUP_EXPORT_SID_REQUESTED;
	}
       ...
       "Space separated route target list (A.B.C.D:MN|EF:OPQR|GHJK:MN)\n")
{
	...
	if (!yes && !ep->tovpn_sid_ready)
		/* RT half cleared, leak gates close — but the SID is still
		 * valid for the VRF.  Drop only the in-flight REQUESTED latch
		 * so a later reconfig can re-issue the request.
		 */
		ep->flags &= ~BGP_MUP_EXPORT_SID_REQUESTED;
...
DEFPY (af_sid_mup_export,
       ...
       "[no] sid mup export <auto$sid_auto|explicit$sid_explicit X:X::X:X$sid_value>",
       NO_STR
       ...
       "SID value\n")
{
	...
	if (yes) {
		...
		/* Operator explicitly removes SID config — release the
		 * (behavior, vrf_id) request from zebra so it can free its
		 * bookkeeping for both the in-flight and ready cases.  The
		 * SID was auto-allocated; explicit values are kernel-pinned
		 * by the operator and don't need a release.
		 */
		if (!(ep->flags & BGP_MUP_EXPORT_SID_EXPLICIT) &&
		    bgp->srv6_locator_name[0] != '\0' &&
		    (ep->tovpn_sid_ready ||
		     (ep->flags & BGP_MUP_EXPORT_SID_REQUESTED))) {
			struct srv6_sid_ctx ctx = {};

			ctx.behavior = (afi == AFI_IP)
					       ? ZEBRA_SEG6_LOCAL_ACTION_END_M_GTP4_E
					       : ZEBRA_SEG6_LOCAL_ACTION_END_M_GTP6_E;
			ctx.vrf_id = bgp->vrf_id;
			bgp_zebra_release_srv6_sid(&ctx, bgp->srv6_locator_name);
		}
		ep->flags &= ~(BGP_MUP_EXPORT_SID_AUTO | BGP_MUP_EXPORT_SID_EXPLICIT |
			       BGP_MUP_EXPORT_SID_REQUESTED);
...
void bgp_mup_export_config_write(struct vty *vty, struct bgp *bgp, afi_t afi, int indent)
{
	...
	if (ep->flags & BGP_MUP_EXPORT_SID_AUTO)
		vty_out(vty, "%*ssid mup export auto\n", indent, "");
	else if (ep->flags & BGP_MUP_EXPORT_SID_EXPLICIT)
		vty_out(vty, "%*ssid mup export explicit %pI6\n", indent, "",
...
void bgp_mup_replay_origins_all(void)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bm->bgp, node, bgp)) {
		 ...
		 * configured before the locator was available: re-request the
		 * shared End.M.GTP{4,6}.E SID and let the SID-arrival callback
		 * replay update_all over the unicast RIB.
		 */
		for (afi = AFI_IP; afi < AFI_MAX; afi++) {
			struct bgp_mup_export_policy *ep = bgp_mup_export_peek(bgp, afi);
			...
			if (ep->tovpn_sid_ready)
				continue;
			(void)mup_export_request_sid_if_needed(bgp, afi, ep);
		}
