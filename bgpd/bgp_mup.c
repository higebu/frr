			      ...
			      const struct in6_addr *sid_value)
{
...

	{
		...
		if (!ep || (ep->flags & BGP_MUP_EXPORT_SID_EXPLICIT)) {
			 ...
			 * back to zebra so the binding doesn't leak.
			 */
       ...
       "SID value\n")
{
	...
	if (yes) {
		 ...
		 * (behavior, vrf_id) binding from zebra so it can free its
		 * bookkeeping.  The SID was auto-allocated; explicit values
		 * are kernel-pinned by the operator and don't need a release.
		 */
		if (!(ep->flags & BGP_MUP_EXPORT_SID_EXPLICIT) &&
		    bgp->srv6_locator_name[0] != '\0' && ep->tovpn_sid_ready) {
			struct srv6_sid_ctx ctx = {};
