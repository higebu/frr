...
/* Per-(vrf, afi) MUP policy.  At this stage carries only the import/export
 * RT lists; later commits grow the struct with RD, SID, segment, and
 * scalar-DSD knobs as the originate path lands.  Mirrors L3VPN's
 * vpn_policy[afi] one slot at a time so a vrf doing both L3VPN and MUP
 * can keep the two policies independent.
 */
				     ...
				     struct zapi_route *api)
{
	 ...
	 * the SR locator interface so zebra emits RTA_OIF + encap seg6local
			       ...
			       const struct prefix_mup *pm, const struct mup_prefix *mp)
{
	...
	if (mp->route_type == BGP_MUP_T1ST_ROUTE) {
		 ...
		 */
		bgp_mup_zapi_init(&api, bgp, info, true);
		bgp_mup_build_t2st_route(mp, dsd, install_vrf_id, &api);
	}
	/* Route prefix lands in the per-vrf RIB; nexthop vrf stays in
	 * default (the SR underlay).
	 */
...
/* No `address-family ipv[46] mup`-only writeback yet.  All VRF-local
 * MUP policy lives under unicast AF; this MUP-AF function is reserved
 * for SAFI-global knobs (e.g. `bgp retain route-target all`) added
 * in later commits.
 */
