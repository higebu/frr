 ...
 * `address-family ipv[46] mup` on a non-default-VRF instance via
 * `rd ASN:NN`, `rt <import|export|both> RTLIST`, and
 * `sid <auto|explicit X:X::X:X> [locator NAME]`.
 *
 ...
 * via `segment interwork` under the MUP AF.
 */
#define BGP_MUP_EXPORT_SEGMENT_INTERWORK (1U << 4)
/* Operator explicitly enabled DSD origination for this (vrf, afi)
 * via `segment direct` under the MUP AF (sub-block holds address /
 * behavior / segment-id).
 */
#define BGP_MUP_EXPORT_SEGMENT_DIRECT (1U << 5)
/* DSD address was set explicitly via the `segment direct` sub-block's
 * `address A.B.C.D` (otherwise default to router-id).
 */
#define BGP_MUP_EXPORT_DSD_ADDRESS_SET (1U << 6)
	 ...
	 * configured via `route-map <import|export> RMAP` under
	 * `address-family ipv[46] mup`.  The TOMUP slot filters
	...
	/* DSD scalar (`segment direct` sub-block) origination metadata.
	 ...
	 * the sub-block's `address A.B.C.D`; the End.DT* behavior is
	 * configured via `behavior <dt4|dt6|dt46>`; the MUP extended
	 * community is configured via `segment-id ASN:NN`.  RD/RT/SID
	 * come from the shared `rd` / `rt export` / `sid` knobs on the
	 * enclosing MUP-AF policy.
	 */
...
/*
 * Render a BGP-MUP NLRI into JSON for `show bgp ipv[46] mup ... json`.
 * Mirrors bgp_evpn_route2json(): adds route-type-keyed payload fields
 * onto the supplied json object.  RD is included because for SAFI_MUP
 * the RD is part of the NLRI key (struct mup_prefix.rd[8]) rather than
 * encoded in a parent dest, so callers cannot reach it via
 * bgp_rd_from_dest() (returns NULL for SAFI_MUP by design).
 */
void bgp_mup_route2json(const struct prefix_mup *p, json_object *json)
{
	...
	json_object_int_add(json, "length", mp->length);

	bgp_mup_prd_from_bytes(&prd, mp->rd);
	prefix_rd2str(&prd, rdbuf, sizeof(rdbuf), ASNOTATION_PLAIN);
	json_object_string_add(json, "rd", rdbuf);

	switch (mp->route_type) {
	case BGP_MUP_ISD_ROUTE:
		family = IS_IPADDR_V4(&mp->isd_route.ip) ? AF_INET : AF_INET6;
		inet_ntop(family, &mp->isd_route.ip.ip.addr, ipbuf, sizeof(ipbuf));
		json_object_string_add(json, "ipFamily", family == AF_INET ? "ipv4" : "ipv6");
		json_object_string_add(json, "ip", ipbuf);
		json_object_int_add(json, "ipLen", mp->isd_route.ip_prefix_length);
		break;
	case BGP_MUP_DSD_ROUTE:
		family = IS_IPADDR_V4(&mp->dsd_route.ip) ? AF_INET : AF_INET6;
		inet_ntop(family, &mp->dsd_route.ip.ip.addr, ipbuf, sizeof(ipbuf));
		json_object_string_add(json, "ipFamily", family == AF_INET ? "ipv4" : "ipv6");
		json_object_string_add(json, "ip", ipbuf);
		break;
	case BGP_MUP_T1ST_ROUTE: {
		const struct mup_t1st_3gpp_5g *g = &mp->t1st_route.t1st_3gpp_5g;

		family = IS_IPADDR_V4(&mp->t1st_route.ip) ? AF_INET : AF_INET6;
		inet_ntop(family, &mp->t1st_route.ip.ip.addr, ipbuf, sizeof(ipbuf));
		json_object_string_add(json, "ipFamily", family == AF_INET ? "ipv4" : "ipv6");
		json_object_string_add(json, "ip", ipbuf);
		json_object_int_add(json, "ipLen", mp->t1st_route.ip_prefix_length);
		json_object_int_add(json, "teid", g->teid);
		json_object_int_add(json, "qfi", g->qfi);
		if (g->endpoint_address_length) {
			int efamily = IS_IPADDR_V4(&g->endpoint_address) ? AF_INET : AF_INET6;
			char ebuf[INET6_ADDRSTRLEN];

			inet_ntop(efamily, &g->endpoint_address.ip.addr, ebuf, sizeof(ebuf));
			json_object_string_add(json, "endpointAddress", ebuf);
			json_object_int_add(json, "endpointAddressLength",
					    g->endpoint_address_length);
		}
		if (g->source_address_length) {
			int sfamily = IS_IPADDR_V4(&g->source_address) ? AF_INET : AF_INET6;
			char sbuf[INET6_ADDRSTRLEN];

			inet_ntop(sfamily, &g->source_address.ip.addr, sbuf, sizeof(sbuf));
			json_object_string_add(json, "sourceAddress", sbuf);
			json_object_int_add(json, "sourceAddressLength", g->source_address_length);
		}
		break;
	}
	case BGP_MUP_T2ST_ROUTE:
		family = IS_IPADDR_V4(&mp->t2st_route.endpoint_address) ? AF_INET : AF_INET6;
		inet_ntop(family, &mp->t2st_route.endpoint_address.ip.addr, ipbuf, sizeof(ipbuf));
		json_object_string_add(json, "endpointAddressFamily",
				       family == AF_INET ? "ipv4" : "ipv6");
		json_object_string_add(json, "endpointAddress", ipbuf);
		json_object_int_add(json, "endpointAddressLength",
...
/* Resolve T1ST endpoint via the ISD discovery cache and synthesize the
 * End.M.GTP*.E SID per RFC 9433 Section 6.6 / Section 6.5.
 */
				     ...
				     const struct bgp_mup_dsd_entry *dsd, struct zapi_route *api)
{
	...
	api_nh = &api->nexthops[0];
	/* H.M.GTP4.D / End.M.GTP6.D are local actions: anchor the route on
	 * an interface in the SR underlay (default vrf), regardless of
	 * which per-vrf table the route prefix lands in.  The kernel uses
	 * this dev to decide which vrf table to look the post-action SRv6
	 * packet's destination up in; using a default-vrf interface keeps
	 * the lookup in the SR underlay even for cross-vrf installs (route
	...
	api_nh->vrf_id = VRF_DEFAULT;
	api_nh->seg6local_action = ep_v4 ? ZEBRA_SEG6_LOCAL_ACTION_H_M_GTP4_D
					 : ZEBRA_SEG6_LOCAL_ACTION_END_M_GTP6_D;
	api_nh->seg6local_ctx.mobile.valid = true;
	if (ep_v4) {
		/* v4: SR Policy is carried in SEG6_LOCAL_NH6 (single SID).
		 * The kernel's H.M.GTP4.D action constructs the outer SRv6
		 * destination from nh6 (locator+function from DSD's SID) +
		 * IPv4 SA bits (v4_mask_len, MSB-aligned at sr_prefix_len) +
		 * Args.Mob.Session (40 bits, last).  v4_mask_len must be 1..32
		 * and leave room for the 40-bit Args.Mob.Session inside the
		 * SID.  Use the endpoint's mask length (32 for IPv4 endpoints)
		 * so the inner gNB IPv4 SA is fully encoded; the End.DT*
		 * endpoint at the remote PE ignores those bits during decap.
		 */
		api_nh->seg6local_ctx.nh6 = dsd->sid;
		api_nh->seg6local_ctx.mobile.v4_mask_len = 32;
	} else {
		/* v6: SR Policy is carried in SEG6_LOCAL_SRH (n-segment SRH).
		 * The kernel's End.M.GTP6.D action augments the configured
		 * SRH by one extra leading slot, stamps the original outer DA
		 * into segments[0], and writes Args.Mob.Session into
		 * segments[1] starting at sr_prefix_len.  Pass DSD's SID as
		 * the single SR Policy segment; net layer at the remote PE
		 * (End.DT6) decaps based on that SID.
		 */
		api_nh->seg_num = 1;
		api_nh->seg6_segs[0] = dsd->sid;
		SET_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_SEG6);
		api_nh->seg6local_ctx.mobile.v4_mask_len = 0;
	}
...
/* True iff a `segment direct` line on any per-vrf bgp instance owns
 * the MUP-EC Direct-Type Segment Identifier carried on a T2ST.  Used
 * to skip install for self-originated DSDs: the resolved nexthop is a
 * local End.DT* action that would black-hole if a copy of the T2ST
 * came back over the BGP session.
 */
 ...
 *
 * Auto-activation: presence of an FROMMUP rtlist entry on the policy is
 * the activation predicate; no separate master toggle is consulted.
 */
			       ...
			       const struct prefix_mup *pm, const struct mup_prefix *mp)
{
	...
	struct ecommunity *route_rt = info->attr ? bgp_attr_get_ecommunity(info->attr) : NULL;
	struct listnode *node;
	struct bgp *vrf_bgp;
	int last_ret = 0;
	...
	/* No RT extcomm or no per-vrf instance imports any of the route's
	 * RTs — there is no local table the install should land in.
	 */
	if (!route_rt || !bgp_mup_any_vrf_imports(afi, info->attr)) {
		if (BGP_DEBUG(zebra, ZEBRA))
	...
	/* Loop over every per-VRF MUP instance whose `rt import` set
	 * intersects the route's RTs and install in each.  Mirrors
	 * L3VPN's vpn_leak_to_vrf_update — a single VPN/MUP NLRI carrying
	 * multiple RTs is expected to land in every importing VRF.
	 * Nexthop vrf stays in default (the SR underlay) regardless of
	 * which per-vrf table the prefix lands in: bgp_mup_build_t{1,2}st_
	 * route sets api->nexthops[0].vrf_id = VRF_DEFAULT so the
	 * seg6/seg6local dev resolves against the default-vrf locator
	 * interface even for cross-vrf installs.
	 */
	for (ALL_LIST_ELEMENTS_RO(bm->bgp, node, vrf_bgp)) {
		int ret;

		if (vrf_bgp->vrf_id == VRF_DEFAULT || vrf_bgp->vrf_id == VRF_UNKNOWN)
			continue;
		if (!bgp_mup_route_rt_in_import(vrf_bgp, afi, route_rt))
			continue;

		api.vrf_id = vrf_bgp->vrf_id;
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("BGP-MUP: announcing route_type %u install vrf %u to zebra",
				   pm->prefix.route_type, vrf_bgp->vrf_id);
				   ...
				   const struct mup_prefix *mp)
{
	struct zapi_route api = {};
	afi_t afi = bgp_mup_afi_from_prefix(mp);
	struct ecommunity *route_rt = (info && info->attr) ? bgp_attr_get_ecommunity(info->attr)
							   : NULL;
	struct listnode *node;
	struct bgp *vrf_bgp;
	...
	/* Walk every per-VRF MUP instance whose import RT set intersects
	 * the route's RTs and issue a per-VRF ROUTE_DELETE.  Mirrors
	 * L3VPN's vpn_leak_to_vrf_withdraw.  Without the route_rt (e.g.
	 * info==NULL on some teardown paths) we fall back to issuing the
	 * delete against bgp->vrf_id; zebra is idempotent for non-existent
	 * deletes so this is safe.
	 */
	if (!route_rt) {
...
/* Project a MUP NLRI's inner IP key into a generic struct prefix so that
 * the receive-side route-map (`route-map import` under MUP AF) can be
 * matched with `match ip address prefix-list` against the natural operator-facing
 * field of each route type:
 *   - ISD:  the discovery prefix (10.99.0.0/24)
 *   - DSD:  the originating PE host address
 *   - T1ST: the UE prefix (192.168.1.5/32)
 *   - T2ST: the GTP-U endpoint host address
 * The MUP RIB key itself is AF_MUP (struct prefix_mup) and would not
 * match an ipv4/ipv6 prefix-list directly.
 */
static void bgp_mup_inner_prefix(const struct mup_prefix *mp, struct prefix *out)
{
	...
	switch (mp->route_type) {
	case BGP_MUP_ISD_ROUTE:
		if (mp->isd_route.ip.ipa_type == IPADDR_V4) {
			out->family = AF_INET;
	...
	case BGP_MUP_DSD_ROUTE:
		...
		break;
	case BGP_MUP_T1ST_ROUTE:
		if (mp->t1st_route.ip.ipa_type == IPADDR_V4) {
			out->family = AF_INET;
...
/* Apply the receive-side `route-map import` (rmap[FROMMUP]) of the
 * VRF that imports this NLRI's RT, before any install side-effects in
 * bgp_mup_zebra_announce.  Returns true if the route should proceed to
 * the install dispatch; false if RMAP_DENYMATCH says skip.  No rmap
 * configured (or no matched VRF) is treated as PERMITMATCH.
 *
 * The matched VRF is the same one bgp_mup_match_install_vrf selects for
 * the dataplane install — MUP installs are single-shot (one ISD/DSD
 * cache, one T1ST/T2ST install vrf), so the rmap applied here is the
 * matched-install VRF's, not a per-VRF iteration.  Documented limit:
 * if multiple non-default VRFs declare the same RT under
 * `rt import` (in MUP AF), only the first matched VRF's
 * rmap[FROMMUP] runs.
 *
 * Mirrors L3VPN's early route_map_apply in
 * vpn_leak_to_vrf_update_onevrf (bgpd/bgp_mplsvpn.c) — same semantic
 * ("filter the act of installing a foreign-RD route into local
 * forwarding state"), simpler topology because MUP install is global
 * SRv6 dataplane state rather than a per-VRF FIB.
 */
	...
	for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
		...
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
			if (!CHECK_FLAG(pi->flags, BGP_PATH_SELECTED))
				continue;
			/* Route via zebra_announce so the FROMMUP rmap gate
			 * fires uniformly — bypassing it here would let a
			 * previously denied T1ST/T2ST sneak in on the next
			 * cache mutation.
			 */
			(void)bgp_mup_zebra_announce(dest, pi, bgp);
			break;
		}
	}
}
				       ...
				       const struct in6_addr *nexthop_override)
{
	 ...
	 * mp_nexthop_global zero; with `nexthop A.B.C.D|X:X::X:X`
	 * configured under the MUP AF, the per-(vrf, afi) policy
	 * supplies an explicit value (IPv4 stored as IPv4-mapped) —
	 * same posture as L3VPN's BGP_VPN_POLICY_TOVPN_NEXTHOP_SET path.
	 */
			    ...
			    const struct in6_addr *sid, bool withdraw)
{
	 ...
	 */
	{
		...
		/*
		 * End.M.GTP6.E writes src_addr verbatim into the rebuilt
		 * outer IPv6 SA (RFC 9433 Section 6.5; kernel
		 * input_action_end_m_gtp6_e_finish), so leaving it at ::
		 * makes IPv6 output drop the GTP-U.  v4 only consults
		 * src_addr's trailing bits, so the locator base is correct
		 * for both AFIs.
		 */
		lctx.mobile.src_addr = loc->prefix.prefix;
		/*
...
/* ----------------------------------------------------------------------
 * Origin primitives: drive ISD origination from the VRF SAFI_MUP RIB.
 *
 * Per-(vrf, afi) policy is configured under `address-family ipv[46] mup`;
 * the source set is the locally-originated entries in that VRF's
 * SAFI_MUP RIB (BGP_ROUTE_STATIC from `network`, BGP_ROUTE_REDISTRIBUTE
 * from `redistribute`).  The export-policy DEFPYs use the
 * mup_origin_prechange / _postchange wrappers around their state
 * mutations.  Auto-activation is predicate-only: TOMUP becomes active
 * the moment `rd` is set together with `segment interwork` or a
 * fully-defined `segment direct` sub-block.
 * ----------------------------------------------------------------------
 */

static bool mup_origin_should_skip_prefix(const struct prefix *p)
{
	if (!p)
		return true;
...
/* Centralised export-side gate.  Mirrors L3VPN's vpn_leak_to_vpn_active
 * (bgpd/bgp_mplsvpn.h): a single helper that every VRF→MUP leak entry
 * point routes through, so a new caller can never forget one of the
 * implicit checks.
 *
 * Returns the per-policy struct iff the VRF is configured to leak into
 * BGP-MUP for this AFI; NULL otherwise.  When @pmsg is non-NULL, it is
 * set to a static reason string on the failing branch.
 */
static struct bgp_mup_export_policy *
mup_origin_to_mup_active(struct bgp *bgp_vrf, afi_t afi, const char **pmsg)
{
	struct bgp_mup_export_policy *ep;

	if (!bgp_vrf || bgp_vrf->vrf_id == VRF_DEFAULT) {
		if (pmsg)
			*pmsg = "source bgp instance is the default vrf";
		return NULL;
	}
	if (afi != AFI_IP && afi != AFI_IP6) {
		if (pmsg)
			*pmsg = "afi is neither ipv4 nor ipv6";
		return NULL;
	}
	ep = bgp_mup_export_peek(bgp_vrf, afi);
	if (!ep) {
		if (pmsg)
			*pmsg = "no export policy state";
		return NULL;
	}
	...
	/* Auto-activation: TOMUP requires a configured segment selector;
	 * `rt export` / `rt both` is an originate-side knob (sets the RT on
	 * emitted NLRIs) but is not sufficient on its own.
	 */
	if (!(ep->flags &
	      (BGP_MUP_EXPORT_SEGMENT_INTERWORK | BGP_MUP_EXPORT_SEGMENT_DIRECT))) {
		if (pmsg)
			*pmsg = "no segment selector configured";
		return NULL;
	}
	return ep;
}
				     ...
				     struct bgp_path_info *path_vrf)
{
	...
	if (mup_origin_should_skip_prefix(p))
		return;

	ep = mup_origin_to_mup_active(from_bgp, afi, NULL);
	if (!ep || !(ep->flags & BGP_MUP_EXPORT_SEGMENT_INTERWORK))
		return;

	/* SAFI_MUP locally-originated entries (BGP_ROUTE_STATIC from
	 * `network`, BGP_ROUTE_REDISTRIBUTE from `redistribute` once the
	 * latter grows a SAFI key) are unconditionally valid sources for
	 * ISD origination — bgp_static_update only sets BGP_PATH_VALID
	 * for SAFI_MPLS_VPN/ENCAP/EVPN, and bgp_process selection runs
	 * asynchronously, so requiring VALID here would race with the
	 * SID-arrival replay.  Skip only paths actively being removed.
	 */
...
void mup_leak_from_vrf_update_all(struct bgp *to_bgp, struct bgp *from_bgp, afi_t afi)
{
	...
	/* Walk locally-originated entries in (vrf, afi, SAFI_MUP) — the
	 * canonical source per draft-ietf-bess-mup-safi.  Entries land
	 * here via `network` under the MUP AF (BGP_ROUTE_STATIC) and via
	 * `redistribute` under the MUP AF (BGP_ROUTE_REDISTRIBUTE).  The
	 * sub_type check is the locally-originated filter; the bpi->type
	 * carries the source protocol (ZEBRA_ROUTE_CONNECT, _STATIC,
	 * _OSPF, ...) for redistributed paths and is irrelevant here.
	 * The per-bpi mup_leak_from_vrf_update gates on !REMOVED &&
	 * !suppressed; filtering on SELECTED here would skip paths
	 * pre-bgp_process, which is exactly the situation on SID arrival.
	 */
	for (bn = bgp_table_top(from_bgp->rib[afi][SAFI_MUP]); bn;
...
/* Per-prefix counterpart of mup_leak_from_vrf_update().  Called from
 * bgp_static_withdraw() when a `network <p>` line under
 * `address-family ipv[46] mup` is removed: emit one ISD WITHDRAW for
 * the just-removed (vrf, afi, p) entry, gated on the same active-policy
 * predicate as the update side.  Mirrors L3VPN's
 * vpn_leak_to_vrf_withdraw(pi) hook in bgpd/bgp_route.c.
 */
 ...
 * the `segment direct` sub-block's `address A.B.C.D` on the policy.
 * The MUP-EC (Direct-Type Segment Identifier) is configured via the
 * sub-block's `segment-id ASN:NN`; the End.DT* behavior via the
 * sub-block's `behavior <dt4|dt6|dt46>`.  RD / RT / SID come from
 * the shared `rd` / `rt export` / `sid` knobs already on the
 * enclosing MUP-AF policy.
 */
...
{
	struct bgp_mup_export_policy *ep = mup_origin_to_mup_active(bgp, afi, NULL);
	struct bgp_mup_origin_args args = {};
...
/* Re-evaluate every selected SAFI_MUP path whose matched-install VRF is
 * @vrf_bgp by withdrawing then re-announcing through bgp_mup_zebra_*
 * — withdraw clears any prior cache/install state under the old policy,
 * announce re-applies the (now current) FROMMUP rmap gate.  Used when
 * `route-map import` (under MUP AF) is configured/removed or its
 * body is edited.
 *
 * Mirrors the receive-side replay L3VPN gets for free via
 * vpn_leak_prechange / _postchange + vpn_leak_to_vrf_update_all on each
 * matching VRF; MUP installs are global SRv6 dataplane state so the walk
 * is single-pass over the default-instance MUP RIB rather than per-VRF.
 */
static void mup_import_replay(struct bgp *vrf_bgp, afi_t afi)
{
	...
	for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
		...
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
			...
			if (!pi->attr)
				continue;
			/* Replay every path whose RTs intersect this VRF's
			 * import RTs.  An RT change on any vrf_bgp must
			 * re-evaluate every importing path, even when the
			 * path also imports into other VRFs.
			 */
...
/* MUP-AF policy DEFPYs.  Live under `address-family ipv[46] mup` on a
 * non-default-VRF instance.  Each command wraps state mutation in
 * mup_leak_prechange / _postchange so emitted ISD NLRIs track the
 * policy edit atomically.  Auto-activation is predicate-driven via
 * bgp_mup_origin_active() / bgp_mup_import_active().
 */

static afi_t bgp_mup_export_node2afi(struct vty *vty)
{
	switch (vty->node) {
	case BGP_IPV4_MUP_NODE:
	case BGP_IPV4_MUP_SEGMENT_DIRECT_NODE:
		return AFI_IP;
	case BGP_IPV6_MUP_NODE:
	case BGP_IPV6_MUP_SEGMENT_DIRECT_NODE:
		return AFI_IP6;
	default:
		return AFI_MAX;
	}
...
static int bgp_mup_export_check_ctx(struct vty *vty, struct bgp *bgp, afi_t afi)
{
	if (afi == AFI_MAX) {
		vty_out(vty,
			"%% command only valid under address-family ipv[46] mup\n");
		return CMD_WARNING_CONFIG_FAILED;
	}
	if (bgp->vrf_id == VRF_DEFAULT) {
		vty_out(vty,
			"%% MUP policy must be configured under a non-default vrf bgp instance (`router bgp ASN vrf NAME`); the default-vrf instance only carries the BGP-MUP session\n");
		return CMD_WARNING_CONFIG_FAILED;
	}
	return CMD_SUCCESS;
}
...
DEFPY (af_mup_rd,
       ...
       NO_STR
       "Route distinguisher attached to locally-originated ISD/DSD\n"
...
/* Predicate-only auto-activation: TOMUP/FROMMUP become active the moment
 * the underlying policy fields exist; there is no separate master flag.
 *
 * TOMUP (origination): rd is set AND a segment selector is configured —
 *   either `segment interwork` (ISD) or fully-defined `segment direct`
 *   (DSD: rd + behavior + segment-id).
 * FROMMUP (install of received NLRIs): at least one rt import (or both)
 *   ecommunity is present.  An rd-set+rt-import policy with no segment
 *   selector therefore installs received NLRIs but originates nothing.
 */
bool bgp_mup_origin_active(struct bgp *bgp, afi_t afi)
{
	...
	if (!(ep->flags & BGP_MUP_EXPORT_RD_SET))
		return false;
	return (ep->flags &
		(BGP_MUP_EXPORT_SEGMENT_INTERWORK | BGP_MUP_EXPORT_SEGMENT_DIRECT)) != 0;
}
...
DEFPY (af_mup_rt,
       ...
       NO_STR
       "Route target list for the MUP policy\n"
       "Match RT on received MUP NLRIs (gates per-VRF install)\n"
       "Set RT on locally-originated MUP NLRIs\n"
 ...
 * follows the same shape for parity.  Both directions apply: TOMUP
 * filters at mup_leak_from_vrf_update (export), FROMMUP filters at
 * bgp_mup_zebra_announce (receive-side install gate).
 */
DEFPY (af_mup_route_map,
       af_mup_route_map_cmd,
       "[no] route-map <import|export>$direction_str RMAP$rmap_str",
       NO_STR
       "Route-map applied to MUP policy\n"
 ...
 * already-leaked ISDs (TOMUP) and already-installed receive-side state
 * (FROMMUP).  Mirrors vpn_policy_routemap_update in bgpd/bgp_mplsvpn.c.
 */
...
DEFPY (af_mup_sid,
       af_mup_sid_cmd,
       "[no] sid <auto$sid_auto|explicit$sid_explicit X:X::X:X$sid_value> [locator WORD$loc_name]",
       NO_STR
       "SRv6 SID for the MUP policy\n"
...
DEFPY (af_mup_nexthop,
       af_mup_nexthop_cmd,
       "[no] nexthop [<A.B.C.D|X:X::X:X>$nexthop_su]",
       NO_STR
       "Next-hop carried in locally-originated MUP NLRIs\n"
...
ALIAS (af_mup_nexthop,
       af_no_mup_nexthop_cmd,
       "no nexthop",
       NO_STR
       "Next-hop carried in locally-originated MUP NLRIs\n")
 ...
 */
/* Emit the per-(vrf, afi) MUP-policy lines under
 * `address-family ipv[46] mup`.  All knobs are self-contained: no
 * `mup` infix, no `export` direction qualifier on unidirectional
 * commands, and the `segment direct` sub-block is emitted with
 * two-level indent so the dump round-trips through `vtysh -c
 * 'show running-config'` → re-apply.
 */
void bgp_mup_config_write_af(struct vty *vty, struct bgp *bgp, afi_t afi)
{
	...
	if (ep->flags & BGP_MUP_EXPORT_NEXTHOP_SET) {
		if (ep->tovpn_nexthop_was_v4) {
			...
			vty_out(vty, "%*snexthop %pI4\n", indent, "", &v4);
		} else {
			vty_out(vty, "%*snexthop %pI6\n", indent, "",
				&ep->tovpn_nexthop);
		}
	}

	if (ep->flags & BGP_MUP_EXPORT_SEGMENT_INTERWORK)
		vty_out(vty, "%*ssegment interwork\n", indent, "");

	if (ep->flags & BGP_MUP_EXPORT_SEGMENT_DIRECT) {
		vty_out(vty, "%*ssegment direct\n", indent, "");
		if (ep->flags & BGP_MUP_EXPORT_DSD_ADDRESS_SET &&
		    ep->dsd_address.ipa_type == IPADDR_V4)
			vty_out(vty, "%*saddress %pI4\n", indent + 1, "",
				&ep->dsd_address.ipaddr_v4);
		if (ep->dsd_behavior) {
			const char *bkw = mup_dsd_behavior_code2keyword(ep->dsd_behavior);

			if (bkw)
				vty_out(vty, "%*sbehavior %s\n", indent + 1, "",
					bkw);
		}
		if (ep->dsd_mup_str)
			vty_out(vty, "%*ssegment-id %s\n", indent + 1, "",
				ep->dsd_mup_str);
		vty_out(vty, "%*sexit\n", indent, "");
	}
}
...
void bgp_mup_process_srv6_locator_delete_per_bgp(struct srv6_locator *loc, struct bgp *bgp)
{
	...
	frr_each (bgp_mup_origin_list, bgp_mup_state_origins(bgp), o) {
		...
		/* Withdraw the originated DSD NLRI from the BGP RIB so peers
		 * see the withdraw — mirrors the ISD-side mup_leak_from_vrf_
		 * withdraw_all above and the L3VPN locator-delete path that
		 * withdraws every auto-SID VPN route.  Without this the route
		 * stays advertised after the locator that backs it is gone.
		 */
		...
		/* Tear down the End.DT* / End.M.GTP6.E localsid install whose
		 * locator the operator has removed; otherwise a downstream PE
		 * could continue encapsulating to the now-defunct SID and we
		 * would silently keep terminating those packets.
		 */
		vrf_lo = if_get_vrf_loopback(bgp->vrf_id);
		oif = vrf_lo ? vrf_lo->ifindex : 0;
		plen = loc->block_bits_length + loc->node_bits_length +
		       loc->function_bits_length;
		...
		/* Auto-allocated DSDs: release the SID back to the SID manager
		 * so a recreate-with-different-prefix doesn't leak.  Explicit
		 * SIDs (`sid explicit ...`) are not allocated by the manager,
		 * so there is nothing to release; the locator-arrival replay
		 * will re-issue the install when the new locator covers the
		 * pinned SID.
		 */
...
/* Top-level `segment <interwork|direct>` under MUP AF.  `interwork`
 * is single-line and toggles ISD origination from the SAFI_MUP RIB
 * locally-originated set.  `direct` is the entry point of the
 * `segment direct` sub-block (BGP_IPV[46]_MUP_SEGMENT_DIRECT_NODE)
 * where DSD knobs (address / behavior / segment-id) live.  Both
 * choices are mutually exclusive: setting one clears the other so a
 * (vrf, afi) policy emits at most one of ISD or DSD.
 */
DEFPY(af_mup_segment_interwork,
      af_mup_segment_interwork_cmd,
      "[no] segment interwork",
      NO_STR
      "Origination mode for this MUP policy\n"
      "Interwork Segment Discovery (per-prefix ISD; draft Section 3.1.1)\n")
{
	VTY_DECLVAR_CONTEXT(bgp, bgp);
	afi_t afi = bgp_mup_export_node2afi(vty);
	struct bgp_mup_export_policy *ep;
...
DEFPY_NOSH(af_mup_segment_direct,
	   ...
	   "segment direct",
	   "Origination mode for this MUP policy\n"
	   "Direct Segment Discovery sub-block (single-NLRI per (vrf, afi); draft Section 3.1.2)\n")
{
	VTY_DECLVAR_CONTEXT(bgp, bgp);
	afi_t afi = bgp_mup_export_node2afi(vty);
	struct bgp_mup_export_policy *ep;
...
/* Sub-node DEFPYs under BGP_IPV[46]_MUP_SEGMENT_DIRECT_NODE. */
...
DEFPY(af_mup_segment_direct_segment_id,
      ...
      NO_STR
      "BGP MUP Extended Community, Direct-Type Segment Identifier sub-type (draft Section 3.2)\n"
...
{
	/* MUP-AF policy commands.  Self-contained under
	 * `address-family ipv[46] mup`; no `mup` infix, no `export`
	 * direction qualifier.  Auto-activation is predicate-only.
	 */
	install_element(BGP_IPV4_MUP_NODE, &af_mup_rd_cmd);
	install_element(BGP_IPV4_MUP_NODE, &af_mup_rt_cmd);
	install_element(BGP_IPV4_MUP_NODE, &af_mup_route_map_cmd);
	install_element(BGP_IPV4_MUP_NODE, &af_mup_sid_cmd);
	install_element(BGP_IPV4_MUP_NODE, &af_no_mup_sid_cmd);
	install_element(BGP_IPV4_MUP_NODE, &af_mup_nexthop_cmd);
	install_element(BGP_IPV4_MUP_NODE, &af_no_mup_nexthop_cmd);
	...
	install_element(BGP_IPV6_MUP_NODE, &af_mup_rd_cmd);
	install_element(BGP_IPV6_MUP_NODE, &af_mup_rt_cmd);
	install_element(BGP_IPV6_MUP_NODE, &af_mup_route_map_cmd);
	install_element(BGP_IPV6_MUP_NODE, &af_mup_sid_cmd);
	install_element(BGP_IPV6_MUP_NODE, &af_no_mup_sid_cmd);
	install_element(BGP_IPV6_MUP_NODE, &af_mup_nexthop_cmd);
	install_element(BGP_IPV6_MUP_NODE, &af_no_mup_nexthop_cmd);
	...
	/* `segment direct` sub-block: address / behavior / segment-id. */
