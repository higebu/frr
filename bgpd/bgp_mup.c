...
mup_leak_to_mup_active(struct bgp *bgp_vrf, afi_t afi, const char **pmsg)
{
	struct bgp_mup_export_policy *ep;
	...
	if (!CHECK_FLAG(bgp_vrf->af_flags[afi][SAFI_UNICAST],
			BGP_CONFIG_VRF_TO_MUP_EXPORT)) {
		if (pmsg)
			*pmsg = "export mup not set";
		return NULL;
	}
			      ...
			      struct bgp_path_info *path_vrf)
{
	...
	ep = mup_leak_to_mup_active(from_bgp, afi, NULL);
...
{
	struct bgp_mup_export_policy *ep = mup_leak_to_mup_active(bgp, afi, NULL);
	struct bgp_mup_origin_args args = {};
...
/* Purge every T1ST/T2ST install whose target vrf is @vrf_bgp from the
 * default-vrf MUP RIB.  Called from `no import mup` BEFORE the toggle
 * is cleared, so the route's RT still matches and we can identify
 * installs to withdraw.  zebra silently ignores unknown deletes, so
 * over-broad emission across the whole MUP RIB is safe.
 */
static void mup_import_purge_installed(struct bgp *vrf_bgp, afi_t afi)
{
	struct bgp *default_bgp = bgp_get_default();
	struct bgp_table *table;
	struct bgp_dest *dest;

	if (!default_bgp || !vrf_bgp || vrf_bgp->vrf_id == VRF_DEFAULT)
		return;
	if (afi != AFI_IP && afi != AFI_IP6)
		return;
	...
	for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
		...
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
			...
			if (!CHECK_FLAG(pi->flags, BGP_PATH_SELECTED))
				continue;
			target_vrf = bgp_mup_match_install_vrf(afi, pi->attr);
			if (target_vrf != vrf_bgp->vrf_id)
				continue;
			bgp_mup_zapi_init(&api, default_bgp, pi, false);
			if (!bgp_mup_build_st_delete(&pm->prefix, &api))
				continue;
...
/* Forward decl — definition lives next to the af_imexport_mup DEFPY
 * further down (auto-set helpers logically belong with the toggle).
 */
static void bgp_mup_export_autoset(struct bgp *bgp, afi_t afi);
       ...
       "Route Distinguisher (<as-number>:<number> | <ip-address>:<number>)\n")
{
	...
	bgp_mup_export_autoset(bgp, afi);
...
/* `import mup` / `export mup` master enable toggles — sibling of
 * L3VPN's `import vpn` / `export vpn` (bgp_imexport_vpn_cmd in
 * bgp_vty.c).  Live under `address-family ipv[46] unicast` and gate
 * VRF participation in the MUP leak in either direction so the unicast
 * AF — multi-purpose since the policy moved here — can host
 * RD/RT/SID/RMAP without implicitly triggering origination or install.
 *
 * Backward compat: when the operator has never explicitly typed
 * `[no] export mup` / `[no] import mup`, the first matching policy
 * line auto-sets the corresponding toggle so existing configs continue
 * to work.  Writeback emits the explicit `[no] export mup` / `[no]
 * import mup` line only when the EXPLICIT bit is set; auto-set state
 * round-trips identically without needing a config-write line.
 */
static bool bgp_mup_export_predicted_active(struct bgp *bgp, afi_t afi)
{
	struct bgp_mup_export_policy *ep = bgp_mup_export_peek(bgp, afi);

	if (!ep)
		return false;
	return (ep->flags & BGP_MUP_EXPORT_RD_SET) ||
	       ep->rtlist[BGP_MUP_POLICY_DIR_TOMUP];
}
...
static void bgp_mup_export_autoset(struct bgp *bgp, afi_t afi)
{
	...
	if (bgp->vrf_id == VRF_DEFAULT)
		return;
	if (CHECK_FLAG(bgp->af_flags[afi][SAFI_UNICAST],
		       BGP_CONFIG_VRF_TO_MUP_EXPORT_EXPLICIT))
		return;
	if (!bgp_mup_export_predicted_active(bgp, afi))
		return;
	SET_FLAG(bgp->af_flags[afi][SAFI_UNICAST], BGP_CONFIG_VRF_TO_MUP_EXPORT);
}

DEFPY (af_imexport_mup,
       af_imexport_mup_cmd,
       "[no] <import|export>$direction_str mup",
       NO_STR
       "Install matching MUP NLRIs from default-instance MUP RIB into this VRF unicast forwarding state\n"
       "Originate MUP ISD/DSD from this VRF unicast RIB into default-instance MUP RIB\n"
       "to/from default instance BGP-MUP RIB\n")
{
	VTY_DECLVAR_CONTEXT(bgp, bgp);
	...
	if (bgp->inst_type != BGP_INSTANCE_TYPE_VRF) {
		vty_out(vty,
			"%% import|export mup must be configured under a non-default vrf bgp instance\n");
		return CMD_WARNING_CONFIG_FAILED;
	}
	if (safi != SAFI_UNICAST || (afi != AFI_IP && afi != AFI_IP6)) {
		vty_out(vty,
			"%% import|export mup valid only under address-family ipv4|ipv6 unicast\n");
		return CMD_WARNING_CONFIG_FAILED;
	}

	if (strmatch(direction_str, "export")) {
		flag = BGP_CONFIG_VRF_TO_MUP_EXPORT;
		SET_FLAG(bgp->af_flags[afi][safi],
			 BGP_CONFIG_VRF_TO_MUP_EXPORT_EXPLICIT);
	} else {
		flag = BGP_CONFIG_MUP_TO_VRF_IMPORT;
		SET_FLAG(bgp->af_flags[afi][safi],
			 BGP_CONFIG_MUP_TO_VRF_IMPORT_EXPLICIT);
	}
	...
	if (yes) {
		...
		if (flag == BGP_CONFIG_VRF_TO_MUP_EXPORT) {
			mup_dsd_postchange(afi, bgp);
			mup_leak_postchange(afi, bgp);
		} else {
			mup_st_resolve_reannounce_all(afi);
		...
		if (flag == BGP_CONFIG_VRF_TO_MUP_EXPORT) {
			...
			/* Withdraw existing per-vrf installs while the RT
			 * still matches.  After the flag clears,
			 * bgp_mup_match_install_vrf returns VRF_UNKNOWN
			 * and the announce-only reannounce path can no
			 * longer figure out the destination vrf.
			 */
			mup_import_purge_installed(bgp, afi);
		}
		UNSET_FLAG(bgp->af_flags[afi][safi], flag);
		if (flag == BGP_CONFIG_MUP_TO_VRF_IMPORT)
			mup_st_resolve_reannounce_all(afi);
	}
	return CMD_SUCCESS;
}

void bgp_mup_imexport_config_write(struct vty *vty, struct bgp *bgp, afi_t afi, int indent)
{
	if (afi != AFI_IP && afi != AFI_IP6)
		return;
	if (bgp->vrf_id == VRF_DEFAULT)
...
void bgp_mup_vty_init(void)
{
	 ...
	 */
	install_element(BGP_IPV4_NODE, &af_imexport_mup_cmd);
	...
	install_element(BGP_IPV6_NODE, &af_imexport_mup_cmd);
