			   ...
			   uint32_t addpath_tx_id)
{
	...
	case BGP_MUP_T2ST_ROUTE: {
		...
		addr_octets = (afi == AFI_IP) ? IPV4_MAX_BYTELEN
					      : IPV6_MAX_BYTELEN;
		teid_bits = mp->t2st_route.endpoint_address_length
			    - (addr_octets * 8);
				     ...
				     uint32_t addpath_id)
{
	...
	prefix_len = pfx[8];
	if ((afi == AFI_IP && prefix_len > 32)
	    || (afi == AFI_IP6 && prefix_len > 128)) {
		flog_err(EC_BGP_MUP_ROUTE_INVALID,
				     ...
				     uint32_t addpath_id)
{
	...
	if ((afi == AFI_IP && psize != 12)
	    || (afi == AFI_IP6 && psize != 24)) {
		flog_err(EC_BGP_MUP_ROUTE_INVALID,
				      ...
				      uint32_t addpath_id)
{
	...
	if (ep_len != 32 && ep_len != 128) {
		flog_err(EC_BGP_MUP_ROUTE_INVALID,
			 "%u:%s - Rx BGP-MUP T1ST NLRI bad endpoint length %u",
			 peer->bgp->vrf_id, peer->host, ep_len);
		return -1;
	}
	...
	ext->endpoint_address.ipa_type = (ep_len == 32) ? IPADDR_V4
							: IPADDR_V6;
	...
	if (src_len != 0 && src_len != 32 && src_len != 128) {
		flog_err(EC_BGP_MUP_ROUTE_INVALID,
			 "%u:%s - Rx BGP-MUP T1ST NLRI bad source length %u",
			 peer->bgp->vrf_id, peer->host, src_len);
		return -1;
	}
	...
	if (src_len) {
		...
		ext->source_address.ipa_type = (src_len == 32) ? IPADDR_V4
							       : IPADDR_V6;
				      ...
				      uint32_t addpath_id)
{
	...
	ea_len = pfx[8];
	if ((afi == AFI_IP && ea_len > 64)
	    || (afi == AFI_IP6 && ea_len > 160)) {
		flog_err(EC_BGP_MUP_ROUTE_INVALID,
