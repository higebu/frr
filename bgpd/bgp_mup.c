...
/* Validate the optional TLV region of a T1ST/T2ST route body: true if
 * valid or empty, false if malformed (treat-as-withdraw).  Unknown types
 * only need to be structurally sound; per-type length rules apply only
 * to the route types the TLV is applicable to (draft 3.1.5: Types 1-3
 * apply to ST2).
 */
bool bgp_mup_parse_tlvs(uint16_t route_type, const uint8_t *buf, int len)
{
				       ...
				       uint8_t *pfx, int psize, uint32_t addpath_id)
{
	...
	if (!bgp_mup_parse_tlvs(BGP_MUP_T1ST_ROUTE, pfx + off, psize - off)) {
				       ...
				       uint8_t *pfx, int psize, uint32_t addpath_id)
{
	...
	if (!bgp_mup_parse_tlvs(BGP_MUP_T2ST_ROUTE, pfx + mandatory, psize - mandatory)) {
...
/* Release the process-wide BGP-MUP caches.  The per-vrf IPv6 source
 * memo outlives every bgp instance, so bgp_free() cannot own it;
 * bgp_exit() calls this alongside the other module finishers.
 */
...
static bool bgp_mup_resolve_t1st(struct bgp *bgp, const struct bgp_mup_t1st_arch *ext,
				 struct bgp_mup_t1st_resolved *out)
{
	...
	if (ext->source_address_length == 128) {
		 ...
		 * IPv4 SA, then bits End.M.GTP4.E ignores.  The prefix is
		 * the SR-domain source this speaker would use anyway, cut
		 * at /64 to match the End.M.GTP4.E default extraction
		 * offset.
		 */
		if (bgp_mup_local_v6_source(bgp->vrf_id, &out->outer_sa)) {
			memset(&out->outer_sa.s6_addr[8], 0, 8);
			bgp_mup_sid_set_bits(&out->outer_sa, 64, 32,
					     ntohl(ext->source_address.ipaddr_v4.s_addr));
			out->have_outer_sa = true;
		}
	}
				     ...
				     struct zapi_route *api)
{
	...
	SET_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_SEG6);
	/* H.M.GTP4.D and End.M.GTP6.D build their own SRH, so this is never
	 * programmed and never reported; the seg6 segments below are the SR
	 * Policy, not a headend instruction.  Assign it all the same: the
	 * enum's zero value is H.Insert, and leaving that in place would put
	 * a value nobody chose into the forwarding-state hash key.
	 */
			       ...
			       const struct prefix_mup *pm, const struct mup_prefix *mp)
{
	...
	if (mp->route_type == BGP_MUP_T1ST_ROUTE) {
		...
		if (!bgp_mup_t1st_arch_decode(info->attr, &ext) ||
		    !bgp_mup_resolve_t1st(bgp, &ext, &r)) {
		 ...
		 * Section 6.7 H.M.GTP4.D for v4; Section 6.3 End.M.GTP6.D for
