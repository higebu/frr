			       ...
			       const struct prefix_mup *pm, const struct mup_prefix *mp)
{
	...
	if (mp->route_type == BGP_MUP_T1ST_ROUTE) {
		...
		/* H.M.GTP4.D is v4-only (RFC 9433 Section 6.6).  For an IPv6
		 * T2ST endpoint there is no kernel "H.M.GTP6.D" peer action —
		 * lib/srv6.h enumerates only End.M.GTP6.D / End.M.GTP6.D.Di,
		 * which are MUP-PE-side SID endpoints (not ingress headend
		 * actions) installed via the local DSD's End.M.GTP6.E ISD
		 * origination, not via T2ST receive.  In the SRv6 v6 MUP
		 * architecture the gNB sends GTP-U(v6) destined to a SID
		 * prefix that resolves to End.M.GTP6.D at the MUP-PE; that
		 * SID is what drives decap+encap based on the GTP-U TEID, so
		 * a per-(TEID, QFI) FIB entry indexed by endpoint_address adds
		 * no forwarding state.  Drop the install to avoid emitting an
		 * H.M.GTP4.D-shaped seg6local nexthop against a v6 prefix
		 * (which the kernel either rejects or silently misinterprets).
		 * The T2ST path remains in the BGP RIB for inspection.
		 */
		if (!IS_IPADDR_V4(&mp->t2st_route.endpoint_address)) {
			if (BGP_DEBUG(zebra, ZEBRA))
				zlog_debug("BGP-MUP: T2ST IPv6 endpoint; skip FIB install (End.M.GTP6.D handles decap via local SID)");
			bgp_mup_st_delete_send(bgp, info, mp);
			return 0;
		}
		bgp_mup_zapi_init(&api, bgp, info, true);
	...
	/* Loop over every per-VRF MUP instance whose `rt mup import` set
 ...
 * the receive-side route-map (`route-map mup import`) can be matched
 * with `match ip address prefix-list` against the natural operator-facing
...
/* Apply the receive-side `route-map mup import` (rmap[FROMMUP]) of the
 ...
 * `rt mup import`, only the first matched VRF's rmap[FROMMUP] runs.
 *
				      ...
				      struct bgp_path_info *info)
{
	...
	if (rmap_ret == RMAP_DENYMATCH) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("BGP-MUP: route-map mup import on vrf %s denied %pFX (route_type %u); skip install",
 ...
 * `route-map mup import` is configured/removed or its body is edited.
 *
