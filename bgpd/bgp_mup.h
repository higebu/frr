...
/* Emit `[no] export mup` / `[no] import mup` master toggles under
 * `address-family ipv[46] unicast` when the operator has explicitly
 * typed the toggle.  Sibling of bgp_vty.c's `export vpn` / `import vpn`
 * block in bgp_config_write_family.  Auto-set state (no EXPLICIT bit)
 * is left implicit so existing configs round-trip without an extra
 * line on reload.
 */
void bgp_mup_imexport_config_write(struct vty *vty, struct bgp *bgp, afi_t afi, int indent);

/* True iff a VRF bgp instance participates in BGP-MUP leak in either
 * direction.  Mirrors is_bgp_vrf_mplsvpn (bgpd/bgp_mplsvpn.h) — used by
 * show output and conditional cleanup paths that need a single per-VRF
 * "is this bgp tied to MUP?" check.
 */
